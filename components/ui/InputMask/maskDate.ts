...
export default function createAutoCorrectedDatePipe(
  ...
  const dateFormatArray: any = dateFormat
    .split(/[^dmyHMS]+/)
  ...
  return function (conformedValue: any) {
    ...
    // Check first digit
    dateFormatArray.forEach((format: keyof typeof maxValue) => {
    ...
    let month = 0
    const isInvalid = dateFormatArray.some((format: keyof typeof maxValue) => {
      const position = dateFormat.indexOf(format)
    ...
    // TODO: This if prevent from breaking when `defaultValue` is an invalid date
    // !indexesOfPipedChars.length
    if (isInvalid && !indexesOfPipedChars.length) {
      return false
    }
